...
export default function CustomerRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/public/auth/register", {
        method: "POST",
    ...
    <div className="flex items-center justify-center min-h-[70vh]">
      <div className="w-full max-w-md p-8 bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-gray-100 dark:border-slate-800">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-black text-gray-900 dark:text-white mb-2">Crear Cuenta</h2>
          <p className="text-gray-500 dark:text-slate-400">Sumate para guardar tu historial de compras</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1.5">Nombre Completo</label>
            <input
              type="text"
              required
              value={name}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-colors shadow-lg shadow-indigo-500/30 disabled:opacity-70 mt-4"
          >
        ...
        <p className="mt-8 text-center text-sm text-gray-600 dark:text-slate-400">
          ¿Ya tenés cuenta?{" "}
          <Link to="/ingresar" className="text-indigo-600 dark:text-indigo-400 font-bold hover:underline">
            Iniciá sesión
          </Link>
        </p>
      </div>
